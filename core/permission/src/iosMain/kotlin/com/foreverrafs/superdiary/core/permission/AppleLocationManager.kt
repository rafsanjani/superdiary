    ...
    override fun requestLocation(
        ...
        locationManager.delegate = object : CLLocationManagerDelegateProtocol, NSObject() {
            override fun locationManager(manager: CLLocationManager, didUpdateLocations: List<*>) {
                ...
                val coordinates = location?.coordinate?.useContents { Pair(latitude, longitude) }

                coordinates?.let {
                    logger.i(TAG) {
                        "Emitting new Location: [latitude=${it.first}, longitude=${it.second}]"
                    }

                    onLocation(coordinates.first, coordinates.second)
                }
            }
